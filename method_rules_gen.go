...
var paymentMethodRules = map[string]methodRule{
	"ARS": {codes: []string{"BANK_TRANSFER", "CVU", "QRIS"}, required: []string{"documentNumber", "documentType", "email", "firstName", "lastName"}, byMethod: map[string][]string{"CVU": {"phone"}, "QRIS": {"phone"}}},
	"BDT": {codes: []string{"BD_BKASH", "BD_NAGAD"}, required: []string{"accountName", "email", "mobile"}},
	"BRL": {codes: []string{"PIX"}},
	"CLP": {codes: []string{"KHIPU", "MACH", "PAGO46", "WEBPAY"}, required: []string{"customerEmail", "customerName", "documentNumber", "documentType"}},
	"COP": {codes: []string{"BREB", "NEQUI", "PSE"}, byMethod: map[string][]string{"BREB": {"customerEmail", "customerName", "customerPhone", "documentNumber", "documentType"}}},
	...
	"MXN": {codes: []string{"CASH", "OXXO", "SPEI"}},
	...
	"TRY": {codes: []string{"BANK_TRANSFER"}, required: []string{"customerName"}},
	"USD": {codes: []string{"CASH_APP"}, required: []string{"name", "phone", "email", "ipAddress"}},
}

var payoutMethodRules = map[string]methodRule{
	"ARS": {codes: []string{"BANK_TRANSFER"}, required: []string{"accountNo", "accountType", "documentNumber", "documentType", "email", "firstName", "lastName", "phone"}, optionalNullableStringsByMethod: map[string][]string{"BANK_TRANSFER": {"address"}}},
	...
	"TRY": {codes: []string{"BANK_TRANSFER", "PAPARA"}, required: []string{"accountName", "accountNo"}, byMethod: map[string][]string{"BANK_TRANSFER": {"bankCode", "bankName"}}},
